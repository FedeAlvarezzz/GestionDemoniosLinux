...
`, usuario, req.Ejecutable)
	...
	cmd := fmt.Sprintf(`ssh %s@%s 'echo "%s" | sudo tee /etc/systemd/system/app.service'`, usuario, req.Plantilla, service)
	...
	// convertir disco a multiconexion
